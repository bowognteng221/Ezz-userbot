...
async def bot_ver(event):
    ...
    if which("git") is not None:
        ...
        await event.edit(
            "**⚜-**🔯Ezz-userbot🔯 Versi:** \n "
            f"heads/Ezz-userbot-0-x634i7u1"
            "\n**⚜-**Revisi:**\n "
            f"{revout}"
        )
    else:
...
async def amireallyalive(alive):
    ...
    output = (
        ...
        f"╰✠╼━━━━━━━━━━━━━━━✠╯\n"
        f"[ɢʀᴏᴜᴘꜱ](https://t.me/EzzInfo) | [ᴄʜᴀɴɴᴇʟ](https://t.me/ProjectEzz) | [ᴏᴡɴᴇʀ](https://t.me/reugay) | [ɢɪᴛʜᴜʙ](https://github.com/izzafthni/Ezz-userbot)")
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
            await alive.delete()
...
@kyy_cmd(pattern="(?:Ezzon)\\s?(.)?")
async def amireallyalive(alive):
    await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    output = (
        f"●▬▬▬▬▬▬ஜ۩۞۩ஜ▬▬▬▬▬▬● \n"
        f"✾ 🤴 • `ᴏᴡɴᴇʀ    :`[Ezz](t.me/reugay) \n"
